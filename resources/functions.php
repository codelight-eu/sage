...
if (version_compare('7.1', phpversion(), '>=')) {
    $sage_error('You must be using PHP 7.1 or greater.', 'Invalid PHP version');
}
...
if (version_compare('4.7.0', get_bloginfo('version'), '>=')) {
    $sage_error('You must be using WordPress 4.7.0 or greater.', 'Invalid WordPress version');
}
...
array_map(function ($file) use ($sage_error) {
    ...
    if (!locate_template($file, true, true)) {
        $sage_error(sprintf('Error locating <code>%s</code> for inclusion.', $file), 'File not found');
    }
